...
L.TileLayer.WMS2 = L.TileLayer.Canvas.extend({
    ...
    _reset: function (e) {
	var i;
	console.log("my reset: activeTiles: " + L.activeTiles);
	...
	for(i = 0; i < activeLength; i ++) {
	    ...
	    console.log("Deactivating tile: " + x + "," + y);
	    L.animatedTiles[x][y][z].active = false;
	}
	L.activeTiles = [];
	if (L.activeTiles.length != 0) {
	    console.log("argh");
	}
	console.log("after reset: " + L.activeTiles);
	...
	//this.on("tileunload", this._unloadTile, tile);
	tile.layer = this;
        return tile;
    },
	...
	var y = tilePoint.y;
	var z = -1;
	console.log("Unloading tile: " + x + "," + y);
	...
	if (tile.active) {
	    console.log("tile active: " + x + "," + y);
	    var index = L.activeTiles.indexOf(tile) > -1;
	    if (index > -1) {
		console.log("found active tile in activeTiles");
		L.activeTiles.splice(index,1);
	    }
	}
	L.animatedTiles[x][y][z] = null;

	//console.log("unload tile: " + this);
    },
...
/*	imageObj.onload = function() {
	    loadedCount++;
	    if (loadedCount == 2) {
		_this.setupTile(ctx, imageObj, data, tilePoint, zoom, url); 
	    }
	}*/
	...
	dataObj.onload = function() {
	    ...
	    var tileSize = 256;//_this._layer.options.tileSize;
...
//	    }
	...
	//imageObj.src = url;
	dataObj.src = url + "&datapng=true";
    },

   // _redrawTile: function (tile) { console.log("skipping redraw") },

    setupTile: function (ctx, imageObj, data, tileInfo, zoom, dataUrl) {
	_this = this;
	var x = tileInfo.x;
	var y = tileInfo.y;
	var z = -1;

	//console.log("Setting up tile (" + x + ", " + y + ", " + z + ")");
	//console.log(tileInfo.z);
	...
	if (L.animatedTiles[x][y][z] == null) {
	    ...
	    var _tileInfo = L.animatedTiles[x][y][z];

	    if (dataUrl == _tileInfo.dataUrl && _tileInfo.active == true) { // && tileInfo.visible == false) {
		console.log("Data URL is the same -- skip setup.");
		setup = false;
	    }
	}
	...
	// calculate if nodata tile, minvalue, maxvalue
...
/*
	var noDataTile = false;
	var maxSeconds = Number.MAX_VALUE;
	var minSeconds = Number.MIN_VALUE;
*/
	...
	if (minSeconds == Number.MAX_VALUE) {
	    ...
	    console.log("I am not a nodata tile: " + minSeconds);
	...
	console.log(L.activeTiles);

	var drawOnCanvas = this._getDrawOnCanvas(ctx, imageObj, data, minSeconds, maxSeconds);
...
*/
		...
		console.log("ending animation: " + x + "," + y);
	    }
	}
	console.log("starting animation: " + x + "," + y);
	animate();

    },
    _getDrawOnCanvas: function(ctx, imageObj, data, minSeconds, maxSeconds) {
	var oldThreshold = 0;
	//var newCanvas = document.createElement("canvas");
	//newCanvas.width = newCanvas.height = 256;
	//newCtx = newCanvas.getContext("2d");

	//newCtx.drawImage(imageObj, 0, 0);

	//var imgd = newCtx.getImageData(0, 0, 256, 256);
	//var imagePix = imgd.data;
	...
	var draw = function() {
		...
		if ((oldThreshold <= maxSeconds || threshold <= maxSeconds) &&
		    ...
		    console.log("maxSeconds is: " + maxSeconds);		
		    ...
		    var newImgd = ctx.getImageData(0,0,256,256);
		    newImgd.data.set(pix);
		    ctx.putImageData(newImgd,0,0);
		} /*else {
		    pix = new Uint8ClampedArray(imagePix); 	
		    for (var i = 0, n = data.length; i < n; i += 4) {
			pix[i + 3] = 255;
